...
# One integer variable for each cell
x = {(i,j) : slv.IntVar(1, 9, 'x_%d_%d' % (i,j)) for i in range(n)
                                                 for j in range(n)}
...
for i in range(n):
    for j in range(n):
        if grid[i][j] != 0:
            slv.Add(x[i,j] == grid[i][j])

# Different numbers on each row
for i in range(n):
    for j in range(n-1):
        for k in range(j+1, n):
            slv.Add(x[i,j] != x[i,k])

# Different numbers on each column
for j in range(n):
    for i in range(n-1):
        for k in range(i+1, n):
            slv.Add(x[i,j] != x[k,j])

# Different numbers on each box
for box in boxes:
    for h, (i1, j1) in enumerate(box):
        for k, (i2, j2) in enumerate(box):
            if h < k:
                slv.Add(x[i1,j1] != x[i2,j2])
...
while slv.NextSolution():
    ...
    cell_content = {(i,j) : x[i,j].Value() for i in range(n)
                                           for j in range(n)}
